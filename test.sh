...
nohup python3 test_devoscientist.py $port1 &
echo 'Running node on port:' $port1
sleep 1

echo
echo 'Running node on port:' $port2
nohup python3 test_devoscientist.py $port2 $port1 &
sleep 4

echo
echo 'Running node on port:' $port3
nohup python3 test_devoscientist.py $port3 $port1 &
sleep 8
